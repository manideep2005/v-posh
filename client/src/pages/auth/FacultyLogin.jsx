...
export default function FacultyLogin() {
  ...
  const { kratosLogin, startPushLogin, pollPushLogin } = useAuth();
      ...
      <div className="panel" style={{ width: '100%', maxWidth: '440px', padding: '2.25rem', borderTop: '4px solid #3B82F6' }}>
        ...
        <div style={{ background: 'var(--color-navy-900)', borderRadius: '8px', padding: '1.25rem', marginBottom: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '4px', marginBottom: '1rem', background: 'rgba(255,255,255,0.06)', borderRadius: '6px', padding: '3px' }}>
            ...
            <button onClick={() => { setAuthTab('qr'); setError(''); }} style={{ flex: 1, padding: '0.5rem', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '0.8125rem', fontWeight: '600', fontFamily: 'inherit', background: authTab === 'qr' ? 'rgba(94,234,212,0.2)' : 'transparent', color: authTab === 'qr' ? '#5EEAD4' : '#94A3B8', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.35rem' }}>
        ...
        <button type="button" onClick={() => { setError(''); handleGoogleLogin(); }} disabled={googleLoading || rateLimited || busy} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.65rem', padding: '0.72rem 1rem', backgroundColor: '#FFFFFF', border: '1.5px solid #E2E8F0', borderRadius: '6px', fontSize: '0.9375rem', fontWeight: '600', color: '#1E293B', cursor: (googleLoading || busy) ? 'not-allowed' : 'pointer', opacity: (googleLoading || busy) ? 0.6 : 1, transition: 'border-color 0.15s, box-shadow 0.15s', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', fontFamily: 'inherit' }}>
